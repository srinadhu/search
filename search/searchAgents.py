...
def foodHeuristic(state, problem):
	...
	position, foodGrid = state #Start positon
    	food_coordinates = foodGrid.asList()  #list of food coordinates.
	
	if (len(food_coordinates) == 0): #No food Done.
		return 0;

	else:	
		max_distance = 0
		for food in food_coordinates:
			distance = mazeDistance(position, food, problem.startingGameState)  #mazeDistance b/w each food and position.
			if (max_distance < distance):	
				max_distance = distance
    		return max_distance  #returns the maximum mazeDistance.
